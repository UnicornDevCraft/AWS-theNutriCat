...
function displayMenuRecipes(recipesByDay) {
    ...
    // Set up the colors for the cards
    // and the order of the days
    const colors = [
            ...
            <div class="accordion-item">
                ...
                <div id="collapse-${id}" class="accordion-collapse collapse" aria-labelledby="heading-${id}" data-bs-parent="#shopping-info-accordion">
                    <div class="accordion-body position-relative">
                        ${isShoppingList ? `
                        <button class="btn btn-sm btn-outline-primary position-relative top-0 m-2" onclick="copyToClipboard('content-${id}')">
...
function showTooltip(button, message) {
    ...
    tooltip.style.top = `${rect.top - 30}px`;  // a bit above the button
    tooltip.style.left = `${rect.left + rect.width / 2}px`;
    tooltip.style.transform = "translateX(-50%)";
    tooltip.style.background = "#0d6efd"; // Bootstrap primary color
    tooltip.style.color = "white";
    tooltip.style.padding = "5px 10px";
    tooltip.style.borderRadius = "5px";
    tooltip.style.fontSize = "0.8rem";
    tooltip.style.zIndex = "1055";  // Above Bootstrap elements
...
async function addMenu(menuName) {
    ...
    // to display it in the menu container
    try {
        ...
        if (response.ok) {
            ...
            // Create the end section HTML
            // with container for the the shopping info
